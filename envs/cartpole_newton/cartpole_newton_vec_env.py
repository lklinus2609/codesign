...
def step_rewards_done_kernel(
    ...
    #   energy:         -10.0*action²  (minimize force usage — high weight for co-design signal)
    #   smoothness:     -0.1*(action - prev_action)²  (reward smooth control)
    #   velocity:       -0.01*|ẋ| - 0.005*|θ̇|
    terminated_f = float(terminated)
    r = 1.0 * (1.0 - terminated_f) - 2.0 * terminated_f - 1.0 * theta * theta - 0.1 * x * x - 10.0 * action * action - 0.1 * action_diff * action_diff - 0.01 * wp.abs(x_dot) - 0.005 * wp.abs(theta_dot)
